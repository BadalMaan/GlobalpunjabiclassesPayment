...
export default function PaymentClient({invoice,config}:{invoice:any,config:any}){
 ...
 return <div className="payCard card">
   ...
   <div style={{fontSize:12,fontWeight:800,letterSpacing:2,color:"#d69a1d"}}>GLOBAL PUNJABI CLASSES</div>
   <h1 style={{fontSize:34,marginBottom:8}}>Monthly Fee Payment</h1>
   <p style={{color:"#64748b"}}>This secure payment link is for <b>{names.join(" & ")}</b>.</p>
   {invoice.items?.length>0 && <div className="studentBreakdown">{invoice.items.map((x:any)=><div className="breakRow" key={x.id}><span>{x.name}</span><b>{invoice.currency} {x.amount}</b></div>)}</div>}
   ...
   {method==="PAYPAL" && <div className="notice paymentPanel"><b>PayPal</b><p>Use the official PayPal payment page. PayPal.Me does not provide automatic server-side payment confirmation, so the payment will remain under verification until confirmed.</p><a className="btn btnGold" href={config.paypalPaymentLink} target="_blank" rel="noreferrer">PAY WITH PAYPAL →</a><ManualBox method="PAYPAL" refValue={ref} setRef={setRef} submit={manualSubmit} busy={busy}/></div>}
