...
export async function POST(req:Request){
  try {
    ...
    const order=await razorpayCreateOrder({amountMinor:Math.round(Number(invoice.amount)*100),currency:"INR",receipt:invoice.invoice_number,notes:{invoice_id:invoice.id,student_id:invoice.student_id}});
